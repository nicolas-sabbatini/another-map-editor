...
function baseNode:constructor()
	...
	self.maskShapeStencil = function()
		...
		utils.rect("fill", self.x, self.y, self.w, self.h, r or 0, r or 0, utils.defaultCurveSegments)
	end
end
...
function baseNode:drawBaseRectangle(color, ...)
	if self.style.customLayers then
		return
	end
	...
	local r = math.min(self.w, self.h) * (self.style.cornerRadius or 0)
	love.graphics.setLineWidth(self.style.lineWidth or 1)
	love.graphics.setLineStyle(self.style.lineStyle or "rough")
	love.graphics.setColor(c)
	utils.rect(
		self.style.outline and "line" or "fill",
		x,
		y,
		w,
		h,
...
function baseNode:drawText(color)
	...
	love.graphics.setFont(self.style.font or utils.default_font)
	love.graphics.setColor(color or fgc)
...
function baseNode:performReleaseAction(data)
	...
	if self.pressed then
		if self.pointed then
			if
				self.type == utils.nodeTypes.BUTTON
				or self.type == utils.nodeTypes.IMAGE
				or self.type == utils.nodeTypes.ANIMATION
			then
