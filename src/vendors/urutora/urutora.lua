...
function urutora:draw()
	...
	love.graphics.setFont(utils.default_font)
	love.graphics.setLineStyle(utils.style.lineStyle)
	love.graphics.setLineWidth(utils.style.lineWidth)
...
function urutora:pressed(x, y, button)
	if not (button == utils.mouseButtons.LEFT) then
		return
	end
	self.focused_node = nil
	for _, v in ipairs(self.nodes) do
