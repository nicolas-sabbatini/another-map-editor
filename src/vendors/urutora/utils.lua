...
function utils.isLabel(node)
	return node.type == utils.nodeTypes.LABEL
end
function utils.isPanel(node)
	return node.type == utils.nodeTypes.PANEL
end
function utils.isMulti(node)
	return node.type == utils.nodeTypes.MULTI_OPTION
end
function utils.isImage(node)
	return node.type == utils.nodeTypes.IMAGE
end
function utils.isAnimation(node)
	return node.type == utils.nodeTypes.ANIMATION
end
function utils.isToggle(node)
	return node.type == utils.nodeTypes.TOGGLE
end
function utils.isProgressBar(node)
	return node.type == utils.nodeTypes.PROGRESS_BAR
end
function utils.isSlider(node)
	return node.type == utils.nodeTypes.SLIDER
end
function utils.isButton(node)
	return node.type == utils.nodeTypes.BUTTON
end
function utils.isTextField(node)
	return node.type == utils.nodeTypes.TEXT
end
function utils.isJoy(node)
	return node.type == utils.nodeTypes.JOY
end
...
function utils.needsBase(node)
	return not (
		utils.isAnimation(node)
		or utils.isPanel(node)
		or utils.isLabel(node)
		or utils.isTextField(node)
		or utils.isJoy(node)
		or utils.isImage(node)
		or utils.isProgressBar(node)
	)
end
...
function utils.drawWithShader(node, texture, x, y, data)
	...
	if not node.enabled then
		love.graphics.setShader(utils.disabledImgShader)
	end
	if node.pointed then
		love.graphics.setShader(utils.pointedImgShader)
	end
	if node.pressed then
		love.graphics.setShader(utils.pressedImgShader)
	end
...
function utils.rect(mode, x, y, w, h, rx, ry, segments)
	love.graphics.rectangle(mode, math.floor(x), math.floor(y), math.floor(w), math.floor(h), rx, ry, segments)
end
...
utils.pointedImgShader = love.graphics.newShader([[
...
utils.pressedImgShader = love.graphics.newShader([[
  float brightness = 0.1;
  float contrast = 1.1;
